let quote = document.getElementById("domanda");
let author = document.getElementById("autore");
let btn = document.getElementById("btn");
let container = document.getElementById("card")
let dmd= document.getElementById("dmd");
var options=[]
const Keys = {
    Backspace: 'Backspace',
    Clear: 'Clear',
...
let getQuote = () => {
  card.classList.toggle('slide-in-blurred-bottom');
  setTimeout(function(){
    card.classList.toggle('slide-in-blurred-bottom');
       //card.classList.add('slide-in-blurred-top');
       if (window.localStorage.getItem('materia') === null) {
        window.localStorage.setItem('materia', 'Geometria1_2021')
    } 
    const materia = window.localStorage.getItem('materia')
    console.log(materia)
    $.getJSON("./data/"+materia+".json", function(json) {
        const domande = json; // this will show the info it in firebug console
        //console.log(domande)
        var count = Object.keys(domande).length;
        const rndInt = randomIntFromInterval(1, count)
        const dom=domande['d'+rndInt.toString()]
        dmd.innerText='d'+rndInt.toString();
        quote.innerText = dom.domanda+"."; 
        author.innerText = dom.data;
    });
  
    card.classList.toggle('slide-in-blurred-top');
    setTimeout(function(){
      card.classList.toggle('slide-in-blurred-top');
    }, 1000);
  }, 1000);
...
function filterOptions(options = [], filter, exclude = []) {
  ...
  Combobox.prototype.selectOption = function(index) {
    ...
    this.activeIndex = index;
    window.localStorage.setItem("materia", selected)
  ...
  $.get('./data/list.txt', function(data){
           options=data.split(',')
           console.log(options);
          //options = ['Geometria1_2021', 'ETA_2122','Geometria2_2021','IstAn_2122','Aritmetica_2122', 'Analisi3_2122'];
